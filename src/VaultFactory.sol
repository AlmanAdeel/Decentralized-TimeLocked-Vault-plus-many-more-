...
contract VaultFactory is ReentrancyGuard {
    ...
    function StoreInTimeLockVault(address user, uint256 amountToLock, uint256 unlocktimeInDays, IERC20 token)
        public
    ...
    function StoreInMileStoneVault(address user, uint256 amountToLock, IERC20 token, uint256 unlocktimeInDays)
        public
        returns (uint256 vaultid)
    {
        require(allowedTokens[address(token)], "Token not allowed");
    ...
    function withdrawFromTimeLockVault(uint256 vaultIndex, uint256 vaultId) external nonReentrant {
    ...
    function withdrawFromMilestoneVault(uint256 vaultIndex, uint256 vaultId) external nonReentrant {
        require(vaultIndex < userVaults[msg.sender].length, "Invalid vault index");
        address clone = userVaults[msg.sender][vaultIndex]; // get their specific cloned vault
        MilestoneVault(clone).withdrawFromMilestoneVault(vaultId); // call the logic on it
    }

    function withdrawFromMultiSigVault(uint256 vaultIndex, uint256 vaultId) external nonReentrant {
        require(vaultIndex < userVaults[msg.sender].length, "Invalid vault index");
        address clone = userVaults[msg.sender][vaultIndex]; // get their specific cloned vault
